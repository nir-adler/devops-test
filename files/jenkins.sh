...
echo "jenkins user start cron"
cron
echo "cron start result ${?}"
echo "set-up github-cli credentials"
echo "${GITHUB_TOKEN_DEVOPS_TEST}" > /tmp/login.txt
gh auth login  --with-token < /tmp/login.txt
echo "set-up github-cli credentials result ${?}"
echo "github clone repo"
sleep 3
cd /var/jenkins_home && gh repo clone nir-adler/devops-test
echo "github clone repo result ${?}"
